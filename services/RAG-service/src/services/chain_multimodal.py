...
def set_links(result):
    try:
        ...
        error_log = f"Failed when set links: {error}"
        logger.error(error_log)
        raise Exception(error_log)
...
def get_sources(doc_ids):
    ...
    {result['images'].append(get_image_path(chunk)) if is_base64(chunk.page_content)
     else result['links'].append(get_link(chunk)) for chunk in chunks}
