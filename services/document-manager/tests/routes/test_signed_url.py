...
def test_get_image_success(client, mock_generate_signed_url_success, mock_requests_get):
    ...
    assert response.headers["content-type"] == Utils.CONTENT_TYPE_IMAGE_JPEG
    assert response.content == Utils.FAKE_IMAGE_DATA
    mock_generate_signed_url_success.assert_called_once_with(URLs.EXAMPLE_IMAGE_URL)
    mock_requests_get.assert_called_once_with(URLs.MOCK_IMAGE_URL)
...
def test_get_image_failure(client, mock_generate_signed_url_failure):
    ...
    assert json_resp["status_code"] == StatusCodes.HTTP_STATUS_ERROR
    assert Utils.ERROR_GENERATING_SIGNED_URL in json_resp["error"]


def test_get_link_success(client, mock_generate_signed_url_success):
    response = client.get(URLs.GET_LINK_URL, follow_redirects=False)

    assert response.status_code == StatusCodes.HTTP_STATUS_REDIRECT
    assert response.headers["location"] == URLs.EXPECTED_DOCUMENT_LOCATION
