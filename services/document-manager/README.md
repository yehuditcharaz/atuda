# :helicopter: Ofer knowledge rag - document manager service
...
## :rocket: Overview
There are 2 routers on the server:
### 1. feedback_route
   Reads feedback from the container, creates an Excel file from it, and then downloads it to the user.
...
1. Build the Docker image
   ```powershell
   docker build -t document-manager-service .
   ```
2. Build the Docker container
   ```powershell
   docker run -it -p 5000:5000 --entrypoint bash -v ${pwd}:/app document-manager-service
   ```
